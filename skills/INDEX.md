# Codex Skills & Policy Index

Single place to pick the right skill or reference when working in this repo. Paths are workspace-relative.

## How to Use
- Always load the matching policy from `.cursor/rules/` before executing a skill. Default: `000-overview.md`, plus the domain rule(s) listed below.
- Paste the chosen skill file into Codex. If it has a `details/` folder, open only the files you need (README/examples/checklist/anti-patterns).
- Follow validation steps in the skill; keep outputs in the paths the skill specifies.
- If no skill fits, start from `skills/00-template/skill.md` and align it with the relevant policies.
